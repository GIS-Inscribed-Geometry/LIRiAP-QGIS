"""
LIRiAP Skeleton algorithm wrapper.

Implements a QGIS Processing algorithm that wraps the skeleton-guided
SDF solver from ``skeleton_worker.py``.

This is a fast, BCRS-free solver using medial-axis skeleton decomposition
for seed generation followed by direct SDF-guided boundary expansion.

Algorithm Overview
==================
Wraps ``skeleton_worker._worker_process_feature`` with:
- QGIS parameter handling
- Serial / chunked-parallel / per-feature-parallel execution
- Optional Numba JIT bootstrap (imported from bcrs_worker)
...
See Also
========
skeleton_worker       : Geometric solver
bcrs_algorithm        : Standard BCRS algorithm
bcrs_fast_algorithm   : Optimised BCRS algorithm
"""
...
from bcrs_worker import _NUMBA_AVAILABLE
...
def _process_slice(job_array, start, end, shared_params):
    ...
    for i in range(start, end):
        ...
        out[feat_id] = (wkt, area, angle, ratio,
                        cand_rank, s2_gain, best_effort)
        best_effort_count += int(best_effort)
    return out, best_effort_count


class InscribedRectangleSkeleton(QgsProcessingAlgorithm):
    INPUT = "INPUT"
    OUTPUT = "OUTPUT"
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterVectorLayer(
            self.INPUT, self.tr("Input layer (polygons)"),
            [QgsProcessing.TypeVectorPolygon],
        ))
        self.addParameter(QgsProcessingParameterBoolean(
            self.ALWAYS_RETURN,
            self.tr("Always return a best-effort rectangle"),
            defaultValue=True,
        ))
        self.addParameter(QgsProcessingParameterNumber(
            ...
            defaultValue=5, minValue=1, maxValue=45,
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.GRID_COARSE, self.tr("Coarse grid resolution"),
            QgsProcessingParameterNumber.Integer,
            defaultValue=40, minValue=10, maxValue=1000,
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.GRID_FINE, self.tr("Fine grid resolution"),
            QgsProcessingParameterNumber.Integer,
            defaultValue=120, minValue=30, maxValue=1000,
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.MAX_RATIO,
            self.tr("Max aspect ratio long:short (0=unlimited)"),
            QgsProcessingParameterNumber.Double,
            defaultValue=1.6, minValue=0.0, maxValue=20.0,
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.TOP_K, self.tr("Top-K candidates"),
            QgsProcessingParameterNumber.Integer,
            defaultValue=3, minValue=1, maxValue=20,
        ))
        self.addParameter(QgsProcessingParameterBoolean(
            self.USE_BUFFER, self.tr("Apply containment buffer"),
            defaultValue=False,
        ))
        self.addParameter(QgsProcessingParameterNumber(
            self.BUFFER_VALUE, self.tr("Buffer value in map units"),
            QgsProcessingParameterNumber.Double,
            defaultValue=-0.5, minValue=-1e9, maxValue=1e9, optional=True,
        ))
        self.addParameter(QgsProcessingParameterBoolean(
            self.USE_CHUNKING,
        ...
        self.addParameter(QgsProcessingParameterNumber(
            ...
            QgsProcessingParameterNumber.Integer,
            defaultValue=1, minValue=0, maxValue=512,
        ))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        always_return = self.parameterAsBoolean(
            parameters, self.ALWAYS_RETURN, context)
        angle_step = self.parameterAsInt(
            parameters, self.ANGLE_STEP, context)
        grid_coarse = self.parameterAsInt(
            parameters, self.GRID_COARSE, context)
        grid_fine = self.parameterAsInt(
            parameters, self.GRID_FINE, context)
        max_ratio = self.parameterAsDouble(
            parameters, self.MAX_RATIO, context)
        top_k = self.parameterAsInt(parameters, self.TOP_K, context)
        use_buffer = self.parameterAsBoolean(
            parameters, self.USE_BUFFER, context)
        buf_value = (self.parameterAsDouble(
            parameters, self.BUFFER_VALUE, context)
            if use_buffer else 0.0)
        use_chunking = self.parameterAsBoolean(
            parameters, self.USE_CHUNKING, context)
        auto_install_numba = self.parameterAsBoolean(
            parameters, self.AUTO_INSTALL_NUMBA, context)
        n_workers_in = self.parameterAsInt(
            parameters, self.N_WORKERS, context)
        _, installed_now = ensure_numba(feedback, auto_install_numba)
        if installed_now:
            feedback.pushInfo(
                "Re-run the algorithm to activate numba acceleration.")
        ...
        if not use_parallel or n_workers == 1:
            ...
            for feat_id, wkb_bytes in job_array:
                ...
                res = _worker_process_feature(
                    (feat_id, wkb_bytes, *shared_params))
                if res is not None:
                    (_, wkt, area, angle, ratio,
                     cand_rank, s2_gain, best_effort) = res
            ...
            with _cf.ThreadPoolExecutor(max_workers=n_workers) as exe:
                future_to_size = {
                    exe.submit(_process_slice, job_array, s, e,
                               shared_params): (e - s)
                    for s, e in slices
                }
                done = 0
            ...
            with _cf.ThreadPoolExecutor(max_workers=n_workers) as exe:
                futures = {
                    exe.submit(_worker_process_feature,
                               (fid, wkb, *shared_params)): fid
        ...
        for fid in feat_order:
            if fid not in results:
                continue
            wkt, area, angle, ratio, cand_rank, s2_gain, best_effort = \
                results[fid]
    ...
    def createInstance(self):
        return InscribedRectangleSkeleton()
    ...
    def shortHelpString(self):
        return build_short_help(
            "Skeleton", "skeleton", _NUMBA_AVAILABLE)
