...
def _worker_process_feature(args, emitter=None):
    ...
    try:
        ...
        return (feat_id, rect.wkt, round(area, 4), round(angle, 2), round(ratio, 4))
    except Exception:
        return None
...
def _search(
    ...
    bounds = [(a, _upper_bound(shapely_poly, a, max_ratio)) for a in candidates]
    ...
    if res.fun < -best_area:
        ...
        best_rect, best_area = _solve_axis_rect(rot_poly, grid_steps_fine, max_ratio)
