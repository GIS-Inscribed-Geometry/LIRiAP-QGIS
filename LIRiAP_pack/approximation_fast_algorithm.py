"""
LIRiAP Approximation Fast algorithm wrapper.

Implements a QGIS Processing algorithm for fast area-focused rectangle search
with optimized execution using slice-based processing.

Algorithm Overview
==================
Wraps the geometric solver in ``approximation_fast_worker.py`` with:
- QGIS parameter handling
- Slice-based parallel execution (reduced overhead)
- Optional Numba JIT bootstrap

Execution Modes
===============
- Serial (N_WORKERS=1): Single-threaded execution
- Parallel (N_WORKERS>1): Slice-based parallel (recommended)
...
See Also
========
approximation_fast_worker: Geometric solver
approximation_standard_algorithm: Non-optimized variant
"""
...
class InscribedRectangleApproximationFast(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                self.MAX_RATIO,
                "Max aspect ratio long:short (0=unlimited)",
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.REFINE_BUFFER,
                "Enable containment refinement buffer",
                defaultValue=False,
            )
        )
        self.addParameter(
            QgsProcessingParameterNumber(
                self.REFINE_BUFFER_VALUE,
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                ...
                "Attempt safe numba auto-install if missing",
                defaultValue=False,
            )
        )
        self.addParameter(
            QgsProcessingParameterNumber(
                self.N_WORKERS,
                "Workers (0 = auto, 1 = serial, >1 = custom)",
                QgsProcessingParameterNumber.Integer,
                defaultValue=1,
                minValue=0,
                maxValue=128,
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(self.OUTPUT, "Inscribed rectangles")
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        buf_value = self.parameterAsDouble(
            parameters, self.REFINE_BUFFER_VALUE, context
        )
    ...
    def name(self):
        return "lir_approximation_fast"

    def displayName(self):
        return "Approximation Fast"
    ...
    def createInstance(self):
        return InscribedRectangleApproximationFast()
    ...
    def shortHelpString(self):
        return build_short_help(
            "Approximation Fast", "approximation_fast", _NUMBA_AVAILABLE
        )
