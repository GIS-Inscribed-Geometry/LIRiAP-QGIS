...
from bcrs_fast_worker import process_slice, _worker_process_feature, _NUMBA_AVAILABLE
...
class InscribedRectangleBCRSFast(QgsProcessingAlgorithm):
    INPUT = "INPUT"
    OUTPUT = "OUTPUT"
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for feat in layer.getFeatures():
            ...
            job_array.append((feat.id(), wkb, len(wkb)))
            feat_order.append(feat.id())
        if not job_array:
            return {self.OUTPUT: dest_id}

        # Sort by feature complexity proxy (WKB size) for better load balance.
        job_array.sort(key=lambda t: t[2], reverse=True)
        job_array = [(fid, wkb) for fid, wkb, _ in job_array]

        # 4) Resolve execution mode from worker count and runtime capabilities.
        total = len(job_array)
        ...
        if _NUMBA_AVAILABLE:
            ...
            executor_note = f"threads ×{n_workers} (numba active — GIL released)"
        else:
            try:
        ...
        feedback.pushInfo(
            f"Processing {total} features — "
            f"{('parallel: ' + executor_note) if use_parallel else 'serial'} ; "
            f"numba={'yes' if _NUMBA_AVAILABLE else 'no'}"
        )
        ...
        elif use_chunking and _NUMBA_AVAILABLE:
    ...
    def name(self):
        return "lir_bcrs_fast"

    def displayName(self):
        return self.tr("BCRS Fast")
    ...
    def createInstance(self):
        return InscribedRectangleBCRSFast()
