...
def _fast_path_solve(poly, max_ratio=0.0):
    ...
    if nv == 4 and not has_holes:
        for i in range(4):
            ...
            angle_deg = (
                math.degrees(math.atan2(frame[3], frame[2])) % 90.0 if frame else 0.0
            )
...
def _solve_from_seed(poly, centroid, angle_deg, max_ratio):
    rot_poly = shp_rotate(poly, -angle_deg, origin=centroid, use_radians=False)
    ...
    rect_world = shp_rotate(rect_rot, angle_deg, origin=centroid, use_radians=False)
...
def _refine_candidates(poly, candidates, max_ratio, emitter=None):
    ...
    for rank, cand in enumerate(candidates):
        ...
        for delta in (-_DELTA_DEG, _DELTA_DEG):
            r2, a2 = _solve_from_seed(poly, centroid, best_angle + delta, max_ratio)
        ...
        if emitter:
            ...
            emitter.emit(
                ...
                rect=[float(eb[0]), float(eb[1]), float(eb[2]), float(eb[3])],
                area=round(cert_a, 4),
        ...
        w_l = math.hypot(coords[1][0] - coords[0][0], coords[1][1] - coords[0][1])
        h_l = math.hypot(coords[2][0] - coords[1][0], coords[2][1] - coords[1][1])
    ...
    try:
        ...
        if fast is not None:
            ...
            if fp_area > 0:
                ...
                w_fp = math.hypot(
                    coords_fp[1][0] - coords_fp[0][0], coords_fp[1][1] - coords_fp[0][1]
                )
                h_fp = math.hypot(
                    coords_fp[2][0] - coords_fp[1][0], coords_fp[2][1] - coords_fp[1][1]
                )
                fp_ratio = (
                    max(w_fp, h_fp) / min(w_fp, h_fp) if min(w_fp, h_fp) > 0 else 1.0
                )
            if emitter:
                emitter.emit(
                    phase="RESULT",
                    ...
                    rect=[
                        float(fp_rect.bounds[0]),
                        float(fp_rect.bounds[1]),
                        float(fp_rect.bounds[2]),
                        float(fp_rect.bounds[3]),
                    ],
            ...
            return (
                feat_id,
                fp_rect.wkt,
                round(float(fp_area), 4),
                round(float(fp_angle), 4),
                round(float(fp_ratio), 4),
                0,
                0.0,
                0,
            )
        ...
        result = _refine_candidates(poly, candidates, max_ratio, emitter=emitter)
        if result is None:
            return None
