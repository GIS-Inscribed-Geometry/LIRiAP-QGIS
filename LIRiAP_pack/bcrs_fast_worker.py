...
# --------------------------------------------------------------------------
_shp_contains_vec = None
try:
    from shapely import contains_xy as _shp_contains_vec
...
def _certify_and_adjust(
    ...
    if buf_enabled and buf_value != 0.0:
        cand = final.buffer(buf_value, cap_style="flat", join_style="mitre")
...
def _conservative_inner_fallback(
    ...
    for frac in (0.002, 0.005, 0.01, 0.02):
        inner = poly.buffer(-span * frac, cap_style="flat", join_style="mitre")
...
def _refine_best_candidate(
    ...
    for rank, cand in enumerate(candidates):
        ...
        if buf_enabled and buf_value != 0.0:
            cand_buf = best_r.buffer(buf_value, cap_style="flat", join_style="mitre")
...
def process_slice(
    ...
    for i in range(start, end):
        ...
        if res is None:
            continue
        _, wkt, area, angle, ratio, cand_rank, s2_gain, best_effort = res
